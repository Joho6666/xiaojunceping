...
export default function KnowledgeSettingsPage() {
  ...
  return <main><div className="content settings-page"><button className="btn knowledge-back" onClick={() => router.back()}>← 返回</button><section className="knowledge-hero"><div><div className="eyebrow">AgentScope Knowledge Base</div><h1>AI 生态知识库</h1><p className="muted">把模型、Agent、Skill、MCP、工具和真实项目参考集中在一个可更新的本地目录中，为每次评估提供可追溯的事实来源。</p></div><div className="knowledge-hero-mark" aria-hidden="true"><span>KB</span><i>↗</i></div></section>{message && <div className="settings-message">{message}</div>}
    <section className="knowledge-overview"><div className="knowledge-stat primary"><span>已发布条目</span><strong>{status?.count ?? "—"}</strong><small>可用于项目匹配</small></div><div className="knowledge-stat"><span>精确模型</span><strong>{status?.modelCount ?? "—"}</strong><small>使用真实 Model ID</small></div><div className="knowledge-stat"><span>更新方式</span><strong>每日同步</strong><small>官方源 + 实时检索</small></div><div className="knowledge-stat"><span>本机能力</span><strong>{discovered.length ? `${discovered.length} 待确认` : "未扫描"}</strong><small>扫描后人工确认</small></div></section>
